...
import sys
import os
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "../..")))  # Fix src import
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            if not self.validate_number_of_columns(test_df):
                validation_error_msg += "Column count mismatch in testing data. "
            ...
            if not self.is_column_exist(test_df):
                validation_error_msg += "Missing required columns in testing data."
...
# Entry Point for DVC Execution
if __name__ == "__main__":
    from src.entity.config_entity import DataIngestionConfig, DataValidationConfig
    from src.data.data_ingestion import DataIngestion

    try:
        # Step 1: Run ingestion to generate train/test files
        ingestion = DataIngestion(DataIngestionConfig())
        data_ingestion_artifact = ingestion.initiate_data_ingestion()

        # Step 2: Define validation config
        data_validation_config = DataValidationConfig(
            data_validation_dir=os.path.join("artifacts", "data_validation"),
            validation_report_file_path=os.path.join("artifacts", "data_validation", "report.json")
        )

        # Step 3: Run validation
        validation = DataValidation(data_ingestion_artifact, data_validation_config)
        validation_artifact = validation.initiate_data_validation()

        # Step 4: Create status.txt for DVC
        status_file_path = os.path.join(data_validation_config.data_validation_dir, "status.txt")
        os.makedirs(os.path.dirname(status_file_path), exist_ok=True)
        with open(status_file_path, "w") as f:
            f.write("Success" if validation_artifact.validation_status else "Failed")
